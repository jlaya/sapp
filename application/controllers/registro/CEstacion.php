...
class CEstacion extends SI_Controller {

    private $dir   = 'registro/';
    private $files = array('new'=>'estacion');
    private $vista = '';
    private $table = 'estacion_policial';
    public function __construct()
    {
        parent::__construct();
        $this->files = (object)$this->files;
        $this->load->model('registro/MEstacion','estacion');
        //echo strtolower(substr($this->router->fetch_class(),1));
    }
    public function index()
    {
        $datos['id']        = $this->modulo->lastId($this->table);
        $datos['lista']     = $this->estacion->listar();
    ...
    public function eliminar()
    {
        ...
        $id = $this->input->get('id');
        $resultado = $this->estacion->eliminar($id);
        if($resultado){
    ...
    public function modificar()
    {
        if ($this->input->post('token') && $this->input->post('token') == $this->session->userdata('token')) {
            ...
            $result = $this->estacion->modificar($data);
            if($result){
                $response_data['success']='ok';
    ...
    public function guardar()
    {
        if ($this->input->post('token') && $this->input->post('token') == $this->session->userdata('token')) {
            $data = $this->input->post();
            $response_data['success']='error';
            $result = $this->estacion->guardar($data);
            if($result){
                $response_data['success']='ok';
