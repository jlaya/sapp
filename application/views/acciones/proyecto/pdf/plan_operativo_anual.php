...
if ($dato[0] == 1 AND $dato[1] == 4) {
    ...
    $title  = "Plan Operativo Anual Estadal";
...
//            $this->pdf->Cell(180.2, 5, utf8_decode('ACTIVIDADES DE LA ACCIÓN ESPECÍFICA'), 'TBLR', 1, 'C', '1');
            ...
            $this->pdf->Cell(15, 5, utf8_decode("$total_imp"), 'TBLR', 0, 'C', '1');
            $this->pdf->SetTextColor(0, 0, 0);  # COLOR DEL TEXTO
            $this->pdf->SetFillColor(255, 255, 255);
            $this->pdf->Cell(130, 5, '', 'TBLR', 0, 'L', '1');
            $this->pdf->Cell(35, 5, $this->pdf->Format_number($montop), 'TBLR', 1, 'R', '1');
            $query_imp_mod         = "SELECT p.codigo,p.partida_presupuestaria,imp.m_asig AS monto FROM acciones_registro AS acc INNER JOIN imp_presupuestaria_modificado AS imp ON(acc.id=imp.id_acc_reg) INNER JOIN partida_presupuestaria AS p ON(imp.partida_id=p.id) WHERE imp.id_acc_reg = $id_acc AND imp.m_asig > 0.00 AND imp.accion = 1 ORDER BY p.codigo ASC";
            $dis_actividad_imp_mod = $this->ModelStandard->query_set($query_imp_mod, 'result');
            if($dato[2] == 5 and count($dis_actividad_imp_mod) > 0){

            // IMPUTACIONES FUENTE DE FINANCIAMIENTO MODIFICADO
            $this->pdf->SetTextColor(255, 255, 255);  # COLOR DEL TEXTO
            $this->pdf->SetFillColor(139, 28, 28);
            $this->pdf->Cell(180.2, 5, utf8_decode('IMPUTACIÓN ACCIONES MODICADO'), 'TBLR', 1, 'C', '1');
            $this->pdf->SetTextColor(0, 0, 0);  # COLOR DEL TEXTO
            $this->pdf->SetFillColor(255, 255, 255);
            $this->pdf->Cell(15, 7.5, utf8_decode('Código'), 'LR', 0, 'C', '0');
            $this->pdf->Cell(165, 7.5, utf8_decode('Denominación'), 'LRTB', 1, 'C', '0');
            ...
            foreach ($dis_actividad_imp_mod as $row) {
                if ($row->monto > 0.00) {
                    $this->pdf->SetFont('Arial', '', 6);
                    $this->pdf->SetTextColor(0, 0, 0);  # COLOR DEL TEXTO
                    $this->pdf->SetFillColor(255, 255, 255);
                    $this->pdf->Cell(15, 5, utf8_decode($this->pdf->string_pad($row->codigo, 13, ".00")), 'TBLR', 0, 'C', '1');
                    $this->pdf->Cell(130, 5, utf8_decode($row->partida_presupuestaria), 'TBLR', 0, 'L', '1');
                    $this->pdf->Cell(35, 5, $this->pdf->Format_number($row->monto), 'TBLR', 1, 'C', '1');
                    $monto_imp_m +=$row->monto; // Sumatoria total de los montos por partidas presupuestarias (Distribucion presupuestaria)
                }
            }

            $monto_sub_total += (float)$montop + (float)$monto_imp_m;

            // echo str_pad("4.02",  13, ".00"); exit;
            ...
            $this->pdf->SetFillColor(255, 255, 255);
            $this->pdf->Cell(130, 5, '', 'TBLR', 0, 'L', '1');
            $this->pdf->Cell(35, 5, $this->pdf->Format_number($monto_imp_m), 'TBLR', 1, 'R', '1');
        }
        ...
        if($dato[2] == 5){
            ...
            $this->pdf->Cell(15, 5, utf8_decode("$total_gen"), 'TBLR', 0, 'C', '1');
            $this->pdf->SetFont('Arial', '', 6);
            $this->pdf->SetTextColor(0, 0, 0);  # COLOR DEL TEXTO
            $this->pdf->SetFillColor(255, 255, 255);
            $this->pdf->Cell(130, 5, '', 'TBLR', 0, 'L', '1');
            $this->pdf->Cell(35, 5, $this->pdf->Format_number($monto_sub_total), 'TBLR', 1, 'R', '1');
        }
...
$title_mod = "";
if($dato[2] == 5){
    $title_mod = "Modificado";
}
$this->pdf->Output("Plan Operativo Anual Estadal ($ano) $title_mod.pdf", 'I');
