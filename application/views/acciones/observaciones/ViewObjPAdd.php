    ...
    $(document).ready(function () {
        ...
        $("#registrar").click(function (e) {
            ...
            if ($('#estatus').val().trim() == '') {
                ...
                $.post('<?php echo base_url(); ?>acciones/observaciones/ControllersObservacionesProy/guardar', $('#form_observaciones_proyecto').serialize(), function (response) {
                    ...
                    if (response == "existe") {
                        ...
                        bootbox.alert("Se registro con exito", function () {
                            ...
                            url = '<?php echo base_url(); ?>acciones/observaciones/ControllersObservacionesProy/procesar/' + $("#accion_id").val() + '';
                            window.location = url;
                        });
                    }
                });
        ...
        $("#organo").change(function () {
            ...
            $.post('<?php echo base_url(); ?>acciones/observaciones/ControllersObservacionesProy/ajax_search/' + organo + '/'+ano_fiscal+'', function (response) {
                ...
                $.each(response, function (i) {
                    if(response[i]['estruc_presupuestaria'] == null){
                        var estruc_presupuestaria = "";
                    }else{
                        var estruc_presupuestaria = response[i]['estruc_presupuestaria'];
                    }
                    //var estruc_presupuestaria = response[i]['estruc_presupuestaria'];
                    //alert(estruc_presupuestaria)
...
//                    }
        ...
        $("#accion_id").change(function () {
            ...
            $.post('<?php echo base_url(); ?>acciones/observaciones/ControllersObservacionesProy/ajax_table_busqueda/' + organo + '', function (response) {
            estructura_id = response['estructura'];
            $("#estructura_id").val(estructura_id);
            //sub_estructura_id = response['estructura_id'].substring(0,9);
            /*alert(estructura_id);
            alert(sub_estructura_id);*/
...
?>
<form id='form_observaciones_proyecto' action="" method="POST">
    <div style="width: 95%;text-transform:uppercase;box-shadow: 0 1px 5px rgba(0,0,0,.85);margin: auto;" class="panel panel-default">
        <div class="panel-heading" style='background: linear-gradient(#2F9292, #3B7272);color:#FFFFFF;'>
            <label style="float: left" class="panel-title " ><!--<a href="<?php echo base_url(); ?>acciones/observaciones/ControllersObservaciones" >Configuraciones</a>-->
                Registrar Observación de Proyecto</label>
            <br>
        </div>
        ...
        <div class="panel-body" hidden="">
            ...
            <div class="col-xs-5">
                <input style="width: 16%" value="<?=date('d-m-Y',now())?>" type="text" class="form-control" />
                <input style="width: 16%" value="<?=date('Y-m-d',now())?>" name="fecha_elaboracion" type="text" class="form-control" />
        ...
        <div class="panel-body">
            ...
            <div class="col-xs-10">
                <textarea id='nom_proyecto' style='width: 65%;' readonly='' class='form-control'></textarea>
            </div>
        </div>
        <div class="panel-body">
                ...
                <div class="panel-body">
                    ...
                    <div class="col-xs-10">
                        <div style="float: left;">
                            <input placeholder="00-00-00-00" type="text" id="estructura_id" name="estructura" class="estructura form-control" style='width: 90%;'/>
                        </div>
                    </div>
                </div>
                <div class="panel-body">
