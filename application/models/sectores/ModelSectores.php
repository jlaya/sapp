 ...
 * @author jesus
 */
class ModelSectores extends CI_Model
{
    ...
    {   $query = $this->db->order_by("codigo", "desc");
    ...
    {
        $result = $this->db->where('sector =', $datos['sector']);
        $result = $this->db->or_where('codigo = ', $datos['codigo']);
        $result = $this->db->get('sectores');

        if ($result->num_rows() > 0) {
            #echo "CORRECTO";
            echo '1';
        } else {
            $result = $this->db->insert("sectores", $datos);
            return $result;
        }
    }
    ...
    public function actualizar($datos)
    {
        ...
        $result = $this->db->update('sectores', $datos);
        return $result;
    }
